...
#include "triangle2d_split_colors.h"
...
void IdleGL(void)
{
    static float  mix_value         = 6.0; // tile_size
    static double totalTimeScript   = 0.0;

    totalTimeScript    += GpuTimer(win_name);
    if(totalTimeScript >= (float)KILL_APP_PERIOD)
        KeyboardGL(ESC_KEY);

// Update Dynamic Values (Uniform)
    shading.mValue    += mix_value;
    if(shading.mValue  < 0.0 || shading.mValue > viewport.mWidth)
        mix_value = -mix_value;
    glUniform1f(glGetUniformLocation(program.mID, "uniform_split_value"), shading.mValue);
